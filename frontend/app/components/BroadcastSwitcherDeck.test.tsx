...
it.each(['director', 'graphics'] as const)('retains bounded previews and scene selection in %s', (workspace) => {
  ...
  expect(screen.getByLabelText('Assigned scenes')).toHaveClass('overflow-y-auto', 'max-h-32');
...
it('requires a deliberate second action before fading to black', () => {
  ...
  render(<BroadcastSwitcherDeck {...props} onFadeToBlack={onFadeToBlack} workspace='director' />);

  fireEvent.click(screen.getByRole('button', { name: /arm fade to black/i }));
  expect(onFadeToBlack).not.toHaveBeenCalled();
  fireEvent.click(screen.getByRole('button', { name: /confirm fade to black/i }));
